...
import { getWeaponDamageModel, getWeaponStats, getWeapons } from '@/catalog';
import {
  calculateTTK,
  damageAtRange,
  damagePerShot,
...
describe('a regra do degrau', () => {
  /*
   * A curva da M433: 26,05 até 21 m, 20,67 até 75 m, 17,13 depois. Nas
   * distâncias repetidas — 21 e 75 — vale o degrau que TERMINA ali, e não o
  ...
  it('confere com o exemplo documentado pela fonte', () => {
    // O Analyzer documenta: a NVO-228E lê 35,22 em 9 m e 27,48 em 10 m.
...
describe('tempo de voo', () => {
  ...
  it('bate com a solução fechada do arrasto quadrático', () => {
    // t = expm1(k·d) / (k·v), com k = 0,0035 e v = 630 m/s.
    const esperado = Math.expm1(0.0035 * 50) / (0.0035 * 630);
    expect(flightTime(model, 50)).toBeCloseTo(esperado, 9);
  });

  it('não escolhe coeficiente sozinho — quem chama declara a fonte', () => {
